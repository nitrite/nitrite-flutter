...
void main() {
  group(retry: 3, 'Spatial Intersects False Positive Test Suite', () {
    ...
    test(
        'Test Intersects - Polygon and MultiPoint with overlapping bounding boxes but no intersection',
        () async {
      ...
      var polygon = reader.read(
              'POLYGON ((40486.563 45036.319, 40084.108 44545.927, 39496.171 44938.774, 39889.018 45526.712, 40486.563 45036.319))')
          as Polygon;
      var multipoint = reader.read(
              'MULTIPOINT ((40933.744 45423.275), (40395.332 45612.623), (40574.536 45576.665))')
          as MultiPoint;
      ...
      expect(result.length, 0,
          reason:
              'MultiPoint does not intersect Polygon, should return no results');
    });

    test(
        'Test Intersects - Polygon and Point inside bounding box but outside geometry',
        () async {
      // Create a non-convex L-shaped polygon to test bbox vs geometry
      var lShapedPolygon = reader
          .read('POLYGON ((0 0, 10 0, 10 5, 5 5, 5 10, 0 10, 0 0))') as Polygon;
      var pointInBBoxButOutsidePolygon =
          reader.read('POINT (7 7)') as Point; // In bbox but outside L-shape
      ...
      expect(result.length, 0,
          reason: 'Point is in bounding box but outside polygon geometry');
    });

    test('Test Intersects - Actual intersection should return results',
        () async {
      // Create geometries that actually intersect
      var polygon =
          reader.read('POLYGON ((0 0, 10 0, 10 10, 0 10, 0 0))') as Polygon;
      var pointInside = reader.read('POINT (5 5)') as Point;
      var lineIntersecting =
          reader.read('LINESTRING (-5 5, 15 5)') as LineString;
      ...
      expect(result.length, 2,
          reason: 'Point and LineString both intersect the polygon');
    });
    ...
    test('Test Within - Point in bounding box but outside geometry', () async {
      ...
      expect(result.length, 0,
          reason: 'Point is in bounding box but not within polygon geometry');
    });
  });
}
